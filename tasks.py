...
SOURCE_DIR = ROOT_DIR.joinpath("labop_labware")
...
PYTHON_VERSION = 3.9
CI_PROJECT_NAME = "oso-labware"
CI_REGISTRY_IMAGE = "registry.gitlab.com/https://gitlab.com/opensourcelab/scientificdata/ontologies/openscienceontology/labware"
...
def docker_test(_c, python_version=PYTHON_VERSION):
    ...
    """
    volume_mount = (
        f"--volume {BIN_DIR}:/labop_labware/bin/ --volume {ROOT_DIR}:/labop_labware:rw"
    )
    registry_path = _get_registry_path_str(python_version)
    pytest_arg = f"pytest -v --cov-report xml:/labop_labware/bin/coverage.xml {TEST_DIR}"
    Path(BIN_DIR).mkdir(parents=True, exist_ok=True)
