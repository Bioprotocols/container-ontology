...
CI_REGISTRY_IMAGE			?= ${CI_REGISTRY}/https://gitlab.com/opensourcelab/scientificdata/ontologies/openscienceontology/labware
...
.build_${PYTHON_VERSION}: Dockerfile labop_labware clean__py
	${_CMD_DOCKER_BUILD} \
...
test:	## run pytest on mounted-in tests (change tests, no rebuild!)
	...
	docker run \
			--platform linux/amd64 \
			--volume $(shell pwd)/bin/:/labop_labware/bin/ \
			--volume $(shell pwd):/labop_labware:rw \
			${DOCKER_IMAGE_TAG} \
			pytest --cov-report xml:/labop_labware/bin/coverage.xml tests/
